...
class AppWithoutLogin():
    ...
    def __init__(self, start_tracking, stop_tracking, window):
        ...
        self.window.geometry('%dx%d+%d+%d' % (width, height, pos_x, pos_y))
        ...
        self.single_marker_mode_checkbox = tk.Checkbutton(self.single_marker_frame, variable=self.single_marker_mode, command=self.single_marker_settings_selection)
        ...
        self.marker_cube_mode_checkbox = tk.Checkbutton(self.marker_cube_frame, variable=self.marker_cube_mode, command=self.marker_cube_settings_selection)
    ...
    def single_marker_save(self):
        try:
            ...
            geometry_string = self.center_self.window(error_window, window_height=280, window_width=1100)
    ...
    def marker_cube_settings_set(self):
        ...
        self.cube_side_marker_1.set(self.marker_cube_settings.side_marker_ids[0])
        self.cube_side_marker_2.set(self.marker_cube_settings.side_marker_ids[1])
        self.cube_side_marker_3.set(self.marker_cube_settings.side_marker_ids[2])
        self.cube_side_marker_4.set(self.marker_cube_settings.side_marker_ids[3])
    ...
    def marker_cube_map(self):
        try:
            detection = MarkerCubeMapping(self.cube_id_selection.get(), 
                                        self.calibration_selection.get(), 
                                        self.video_source.current(),
                                        self.cube_markers_length.get(), 
                                        self.cube_up_marker_id.get(),
            ...
            geometry_string = self.center_self.window(error_window, window_height=350, window_width=1100)
            error_window.geometry(geometry_string)
    ...
    def start_tracking(self):
        try:
            ...
            geometry_string = self.center_self.window(error_window, window_height=220, window_width=1100)
            error_window.geometry(geometry_string)
            ...
            geometry_string = self.center_self.window(error_window, window_height=180, window_width=700)
            error_window.geometry(geometry_string)
            ...
            geometry_string = self.center_self.window(error_window, window_height=180, window_width=800)
            error_window.geometry(geometry_string)
    ...
    def author_updated(self):
        ...
        #len(self.database_calibrations), 
        len(self.calibrations_dict_list),
        self.author.get()))
    ...
    def calibrate(self):
        try:    
            ...
            geometry_string = self.center_self.window(error_window, window_height=270, window_width=1100)
            error_window.geometry(geometry_string)
...
class AppWithLogin(AppWithoutLogin):
    ...
    def __init__(self, start_tracking, stop_tracking, window, db, userId):
        ...
        pos_x = (self.window.winfo_screenwidth()/2) - (width/2); pos_y = (self.window.winfo_screenheight()/2) - (height/2)
        self.window.geometry('%dx%d+%d+%d' % (width, height, pos_x, pos_y))
        self.window.resizable(0, 0)

        # Create some room around all the internal frames
        self.window['padx'] = 5; self.window['pady'] = 5

        self.window.grid_rowconfigure(1, weight=1); self.window.grid_rowconfigure(2, weight=1)
